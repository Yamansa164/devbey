...
class CarsWidget extends StatelessWidget {
  CarsWidget({super.key, required this.screen, required this.bloc});
  final Size screen;
  final SearchBloc bloc;
  @override
  Widget build(BuildContext context) {
    final GetMoreCarsBloc getMoreCarsBLoc = GetMoreCarsBloc.getBloc(context);

    getMoreCarsBLoc.controller.addListener(() {
      getMoreCarsBLoc.addListener();
    });
        ...
        body: SafeArea(
          child: Container(
            ...
            child: Column(children: [
              ...
              Expanded(
                child: BlocBuilder<GetMoreCarsBloc, BlocState>(
                    builder: (context, state) {
                  if (state is GetMoreCarsSuccess) {
                    bloc.listcarsModel!.listCarModel
                        .addAll(state.carsModel.listCarModel);
                  }
                  return GridView.builder(
                    controller: getMoreCarsBLoc.controller,
                    ...
                    itemCount: state is GetMoreCarsLoading
                        ? bloc.listcarsModel!.listCarModel.length + 1
                        : bloc.listcarsModel!.listCarModel.length,
                    itemBuilder: (BuildContext ctx, index) {
                      if (index < bloc.listcarsModel!.listCarModel.length) {
                        List<String> photo = bloc
                            .listcarsModel!.listCarModel[index].photos
                            .split(',');
                        ...
                        return Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(bloc.listcarsModel!.listCarModel[index]
                                  .brandName),
                              Divider(
                                color: ColorManage.white,
                                height: screen.height / 50,
                              ...
                              Container(
                                height: screen.height / 20,
                                color:
                                    const Color.fromRGBO(222, 112, 33, 0.808),
                                child: Column(
                                  children: [
                                    Center(
                                      child: Text(
                                        ...
                                        style:
                                            TextStyle(color: ColorManage.white),
                                      ),
                                    ),
                                    Text(' Day(s):',
                                        style: TextStyle(
                        ...
                        if (state is GetMoreDataFaield) {
                          return const SizedBox();
                        } else {
                          return Lottie.asset(AssetJsonManage.loading);
                        }
                      }
                    },
                  );
